...
Clear[MakeRVFamily,MakeRVFamily2,DeleteRVFamily,DeleteRVFamily2,ExecuteRules,
...
MakeVariablesUnique[ HoldPattern[ Sum[x_, L__] ] ] := Module[ {y,s,z,Lp}, y=MakeVariablesUnique[x]; 
	s=SubstitutionList[L]; z=y/. s; Lp=L/.s; Sum[ Evaluate[z], Evaluate[Lp]]]
MakeVariablesUnique[ x_ + y_] := MakeVariablesUnique[x] + MakeVariablesUnique[y];
MakeVariablesUnique[ x_ * y_] := MakeVariablesUnique[x] * MakeVariablesUnique[y];
MakeVariablesUnique[ x_^ n_Integer ] := Module[{rez,i}, rez = 1; For[ i=0, i<n, rez = rez * MakeVariablesUnique[x]; i++]; rez ] /; n>0
...
ExUnique[x_ HoldPattern[Sum[y_, L__]],LI___] := Sum[Evaluate[ExUnique[x y]], L]
ExUnique[Sum[x_, {i_, N_}]^2,L___] := 
  Sum[ Evaluate[ExUnique[x (x /. i -> ip)]], {i, N}, {ip, N}]
ExUnique[x_ y_] := Ex[x y, RVLIST]
Ex[x_ y_, {L___,X_,NAME_,R___}] := ExUnique[x] ExUnique[y] /; MemberQ[x,X[__]] && \
FreeQ[y,X[__]]
...
Ex[x_,L___] := Module[ {y},  y = Expand[x]; If[ x === y, ExBase[x] , ExUnique[y]] ] \
\
...
*)
(*
Syntax::sntxf: "&" cannot be followed by "& FreeQ[y,X[__]]". More\[Ellipsis]
*)
